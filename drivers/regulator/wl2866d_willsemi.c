...
#define wl2866d_debug(message, ...) \
        pr_info("[%s] : "message, __func__, ##__VA_ARGS__)
...
static int wl2866d_driver_register(int index, struct regmap *regmap)
{
	...
	ret = sysfs_create_file(&(wl2866d_dev.pdevice->kobj), &dev_attr_status.attr);
	ret = sysfs_create_file(&(wl2866d_dev.pdevice->kobj), &dev_attr_info.attr);
	ret = sysfs_create_file(&(wl2866d_dev.pdevice->kobj), &dev_attr_enable.attr);
